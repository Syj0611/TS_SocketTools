...
void config_bus(vector<map<uint32_t, map<uint32_t, vector<map<uint64_t, frame_data>>>>> MappingTable)
{
    ...
    {
        for (int chnidx = 0; chnidx < 6; chnidx++)
        {
...
int main(int argc, char *argv[])
{
    ...
    if (Open_ini(argv[1]))
    {
        ...
        int singal = 1;
        int fd;
        if(access("/tmp/IniMapfifo", F_OK)==-1){
            fd = mkfifo("/tmp/IniMapfifo",0664);
            if (fd == -1){
                perror("IniMapfifo error");
            }
        }
        if (!Read_ini_Config(config))
        {
            ...
            fd = open("IniMapfifo",O_WRONLY);
            write(fd, &singal, sizeof(int));
            close(fd);
        }
    ...
    while (1)
    {
        for (int i = 0; i < HandleList.size(); i++)
        {
            ...
            if (ACANFDMsgSize != 0)
            {
                // cout<<ACANMSG.AMsg.FIdentifier<<endl;
