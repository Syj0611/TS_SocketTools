...
namespace ini
{
	...
	public:
		...
		bool section_exists(const char* section)
		{
			std::string tmp_s(section);
			std::map<std::string, std::map<std::string, std::string> >::iterator it;
			it = settings_.find(tmp_s);
		...
		bool key_exists(const char* section,const char* key)
		{
			std::string tmp_s(section);
			std::map<std::string, std::map<std::string, std::string> >::iterator it;
			std::string tmp_i(key);
			it = settings_.find(tmp_s);
			if (it == settings_.end())
			{
				return false;
			}
			std::map<std::string, std::string> k_v = it->second;
			std::map<std::string, std::string>::iterator it_item;
			it_item = k_v.find(tmp_i);
			if (it_item == k_v.end())
			{
